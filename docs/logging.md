...
- `logging.redactPatterns`: list of regex strings that replaces the default string list for log/transcript output. Built-in structural protections for secret-bearing log fields, form bodies, structured authorization headers, and bare AWS secret access keys always apply, including when this list is copied or customized. File and JSON console records apply these protections before JSON encoding. Configured patterns match decoded values and the complete serialized record, including context across fields and arrays. Captures within strings are re-escaped; captures in numbers, booleans, or null replace the scalar with a quoted mask. JSON structure stays valid. For Control UI tool payloads, custom patterns apply on top of the built-in defaults, so adding a pattern never weakens redaction of values already caught by the defaults.
